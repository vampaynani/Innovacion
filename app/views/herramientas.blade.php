		...
		@foreach($files as $file)
			...
			@endif
				<div class="thumb">
					...
					<div class="iconos">
						<a class="fb-share" href="{{ URL::action('HomeController@herramientas') }}/{{ $file->id }}"><img src="{{ Request::root() }}/imgs/facebook-icon.png"></a>
						<a class="tw-share" href="{{ URL::action('HomeController@herramientas') }}/{{ $file->id }}"><img src="{{ Request::root() }}/imgs/twitter-icon.png"></a>
					</div>
				</div>
				<h1>{{ $file->nomArch }}</h1>
				<p>{{ $file->descArch }}</p>
